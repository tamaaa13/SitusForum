...
func (r *repository) CreateComment(ctx context.Context, model posts.CommentModel) error {
	query := "INSERT INTO comments (post_id, user_id, comments_content, created_at, updated_at, created_by, updated_by) VALUES (?,?,?,?,?,?,?)"
...
func (r *repository) GetCommentByPostID(ctx context.Context, postID int64) ([]posts.Comment, error) {
	query := "SELECT c.id, c.user_id, c.comments_content, u.username FROM comments c JOIN users u ON c.user_id = u.id WHERE c.post_id = ?"
